...
To update the database use this command from the Web project folder (replace `PatientHealthRecord` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../PatientHealthRecord.Infrastructure/PatientHealthRecord.Infrastructure.csproj -s PatientHealthRecord.Web.csproj
```
